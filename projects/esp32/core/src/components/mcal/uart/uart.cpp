...
namespace uart_module {
    ...
    esp_err_t uart_t::read(uint8_t* buffer, size_t len, uint32_t timeout_ms) {
        int read_len = uart_read_bytes(port, buffer, len, pdMS_TO_TICKS(timeout_ms));
        if (read_len >= 0) {
            return ESP_OK;
        }
        return ESP_FAIL;
